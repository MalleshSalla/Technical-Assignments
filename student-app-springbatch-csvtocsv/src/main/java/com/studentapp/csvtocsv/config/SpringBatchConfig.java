package com.studentapp.csvtocsv.config;
...
import com.studentapp.csvtocsv.processer.UniqueStudentProcessor;
import com.studentapp.csvtocsv.writer.Previous_Writer;
import com.studentapp.csvtocsv.writer.StudentClassifer;
...
public class SpringBatchConfig {
     ...
     public FlatFileItemWriter<Student> writer_success(){
    	...
    	writer.setResource(new FileSystemResource("src/main/resources/success.csv"));
     ...
     public FlatFileItemWriter<Student> writer_reject(){
    	 FlatFileItemWriter<Student> writer=new FlatFileItemWriter<>();
    	writer.setResource(new FileSystemResource("src/main/resources/reject.csv"));
    	DelimitedLineAggregator<Student> aggregator=new DelimitedLineAggregator<>();
    	aggregator.setDelimiter(",");
     ...
     public FlatFileItemWriter<Student> writer_delta(){
    	 FlatFileItemWriter<Student> writer=new FlatFileItemWriter<>();
    	writer.setResource(new FileSystemResource("src/main/resources/delta.csv"));
    	DelimitedLineAggregator<Student> aggregator=new DelimitedLineAggregator<>();
    	aggregator.setDelimiter(",");
    	BeanWrapperFieldExtractor<Student> extractor=new BeanWrapperFieldExtractor<>();
    	extractor.setNames(Student.getFields());
     ...
     public FlatFileItemReader<Student> reader_previous(){
    	 ...
    	 reader.setResource(new FileSystemResource("src/main/resources/previous.csv"));
     ...
     public FlatFileItemReader<Student> reader_current(){
    	 FlatFileItemReader<Student> reader=new FlatFileItemReader<>();
    	 reader.setResource(new FileSystemResource("src/main/resources/current.csv"));
    	 DefaultLineMapper<Student> mapper=new DefaultLineMapper<>();
    	 DelimitedLineTokenizer tokenizer=new DelimitedLineTokenizer();
    	 tokenizer.setDelimiter(",");
    	 tokenizer.setNames(Student.getFields());
     ...
     public FlatFileItemReader<Student> reader_delta(){
    	 FlatFileItemReader<Student> reader=new FlatFileItemReader<>();
    	 reader.setResource(new FileSystemResource("src/main/resources/delta.csv"));
    	 DefaultLineMapper<Student> mapper=new DefaultLineMapper<>();
    	 DelimitedLineTokenizer tokenizer=new DelimitedLineTokenizer();
    	 tokenizer.setDelimiter(",");
    	 tokenizer.setNames(Student.getFields());
